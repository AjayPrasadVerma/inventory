...
export default function MaterialStockPage() {
  ...
  useEffect(() => {
    cachedGet<{ data: ItemLite[] }>("/items/options")
      .then((r) => {
        setItems(r.data);
        const preset = new URLSearchParams(window.location.search).get("i");
        if (preset && r.data.some((it) => String(it.id) === preset)) setItemId(preset);
      })
  ...
  useEffect(() => {
    ...
    window.history.replaceState(null, "", `?i=${itemId}`);
