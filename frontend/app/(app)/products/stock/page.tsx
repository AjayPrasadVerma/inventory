...
export default function ProductStockPage() {
  ...
  const loadStock = useCallback((id: string) => {
    ...
    setLoading(true);
    setError(null);
    api<{ data: StockData }>(`/products/${id}/stock`)
  ...
  const productOptions = useMemo<ComboOption[]>(
    () => products.map((it) => ({ value: String(it.id), label: it.name })),
    [products],
  );
  ...
  return (
    <div className="w-full pb-16">
      <PageHeader backHref="/products" title="Product Stock" subtitle="On-hand, production and sales in one place." />

      {/* Toolbar: product picker + filters in one row */}
      <Card className="p-4">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="min-w-[10rem] flex-1">
            <Label>Search</Label>
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Party / note…" disabled={!productId} />
          </div>
      ...
      {!productId ? (
        <Card className="mt-4"><EmptyState title="Select a product" hint="Pick a product above to see its stock, production and sales." /></Card>
      ) : loading ? (
        ...
        <section className="mt-4">
            ...
            <span className="flex flex-wrap items-center gap-1.5 text-sm">
              ...
              {stock.onHand.length > 0
                ? stock.onHand.map((o, i) => (
                    <Badge key={i} tone={o.qty > 0 ? "success" : "neutral"}>{`${o.variant ? o.variant + " · " : ""}${fmtQty(o.qty)}`}</Badge>
                  ))
          ...
          <div className="overflow-hidden rounded-xl border border-border">
            <div className="max-h-[62vh] overflow-auto">
              <table className="data-table sticky-head min-w-[720px]">
                <thead>
                  <tr>
                ...
                <tbody>
                  ...
                  {shown.length === 0 && (
                    <tr><td colSpan={6} className="py-6 text-center text-muted">No movements{hasFilter ? " match these filters" : " yet"}.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
