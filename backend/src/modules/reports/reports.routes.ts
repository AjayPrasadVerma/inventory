...
reportsRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // null → CURRENT_DATE, resolved in SQL so the server's day is used rather
    // than whatever day the caller thinks it is.
    res.json({ data: await reportsRepo.dayActivity(date ?? null) });
  }),
);
